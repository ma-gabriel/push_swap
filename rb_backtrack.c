...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/06 00:34:02 by geymat            #+#    #+#             */
/*   Updated: 2024/01/18 09:47:24 by geymat           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_methode	*back_r(int *a, int *b, int count)
{
	...
	if (!rr)
		return (free_null(rb, 0, 0, 0));
	rr->function_id = 4;
	ft_rr_s(a, b, 1);
	actual = methode_newval(a, count - 1);
	if (!actual)
		return (free_null(rb, rr, 0, 0));
	actual->function_id = 3;
	return (max_methode(rb, rr, back_rr(a, b, count), actual));
}
